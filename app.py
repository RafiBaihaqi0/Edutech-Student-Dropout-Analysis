...
LABEL_MAPPING_PATH = "model/label_mapping.joblib"
...
def main():
    ...
    st.caption(
        "Prototype sistem machine learning untuk memprediksi apakah seorang mahasiswa "
        "Jaya Jaya Institut berpotensi **Dropout** atau **Graduate**, berdasarkan model "
        "Logistic Regression yang telah dilatih pada tahap analisis data sebelumnya. "
        "Model ini dilatih khusus pada mahasiswa yang sudah memiliki status akhir "
        "(Dropout/Graduate); status Enrolled tidak digunakan pada pelatihan karena "
        "belum memiliki label akhir, dan justru menjadi salah satu target penggunaan "
        "model ini di masa mendatang."
    )

    model, label_mapping = load_artifacts()
    ...
    if submitted:
        ...
        prediksi_label = label_mapping[prediksi_encoded]
        proba_dropout = model.predict_proba(input_df)[0][1]
        proba_graduate = 1 - proba_dropout
        ...
        proba_df = pd.DataFrame({
            ...
            "Probabilitas": [proba_dropout, proba_graduate],
        ...
        st.caption(
            "Catatan: model ini tidak memprediksi status Enrolled. Probabilitas di atas "
            "menunjukkan kecenderungan mahasiswa menuju Dropout dibandingkan Graduate."
        )
